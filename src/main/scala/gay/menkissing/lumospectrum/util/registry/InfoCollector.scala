...
package gay.menkissing.lumospectrum.util.registry
...
class InfoCollector:
  ...
  def block(name: String, block: Block): BlockBuilder[Unit] =
    this.block(LumoSpectrum.locate(name), block)
  ...
  def item(name: String, item: Item): ItemBuilder[Unit] =
    this.item(LumoSpectrum.locate(name), item)
  ...
  def tag[T](registry: ResourceKey[? <: Registry[T]], name: String): TagBuilder[T, Unit] =
    this.tag(registry, LumoSpectrum.locate(name))
  ...
  def registerDataGenerators(pack: FabricDataGenerator#Pack): Unit =
    pack.addProvider { (output, lookup) =>
      ...
      new DataProvider {
        ...
        override def getName: String = "InfoCollector-based provider for lumospectrum"
      }
    }
...
object InfoCollector:
  val instance: InfoCollector = InfoCollector().addRawLang("itemGroup.lumospectrum", "Lumo Spectrum")
